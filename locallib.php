...
/**
 * Library of functions for newslettertemp outside of the core api
 */
 ...
 */
class newslettertemp_portfolio_caller extends portfolio_module_caller_base {
    ...
    function __construct($callbackargs) {
        ...
        if (!$this->postid && !$this->discussionid) {
            throw new portfolio_caller_exception('mustprovidediscussionorpost', 'newslettertemp');
        }
    }
    /**
     * @global object
    ...
    public function load_data() {
        ...
        if ($this->postid) {
            if (!$this->post = $DB->get_record('newslettertemp_posts', array('id' => $this->postid))) {
                throw new portfolio_caller_exception('invalidpostid', 'newslettertemp');
            }
        }
        ...
        if (!$this->discussion = $DB->get_record('newslettertemp_discussions', $dbparams)) {
            throw new portfolio_caller_exception('invaliddiscussionid', 'newslettertemp');
        }

        if (!$this->newslettertemp = $DB->get_record('newslettertemp', array('id' => $this->discussion->newslettertemp))) {
            throw new portfolio_caller_exception('invalidnewslettertempid', 'newslettertemp');
        }

        if (!$this->cm = get_coursemodule_from_instance('newslettertemp', $this->newslettertemp->id)) {
        ...
        if ($this->post) {
            if ($this->attachment) {
                ...
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_newslettertemp', 'attachment', $this->post->id, 'timemodified', false);
                $embed  = $fs->get_area_files($this->modcontext->id, 'mod_newslettertemp', 'post', $this->post->id, 'timemodified', false);
            ...
            $fs = get_file_storage();
            $this->posts = newslettertemp_get_all_discussion_posts($this->discussion->id, 'p.created ASC');
            $this->multifiles = array();
            foreach ($this->posts as $post) {
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_newslettertemp', 'attachment', $post->id, 'timemodified', false);
                $embed  = $fs->get_area_files($this->modcontext->id, 'mod_newslettertemp', 'post', $post->id, 'timemodified', false);
                $files = array_merge($attach, $embed);
                if ($files) {
    ...
    function get_return_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/newslettertemp/discuss.php?d=' . $this->discussion->id;
    }
    /**
     * @global object
     * @return array
    ...
    function get_navigation() {
        ...
        $navlinks[] = array(
            ...
            'link' => $CFG->wwwroot . '/mod/newslettertemp/discuss.php?d=' . $this->discussion->id,
            'type' => 'title'
        );
    ...
    function prepare_package() {
        ...
        if ($this->attachment) { // simplest case first - single file attachment
            ...
            if ($writingleap) {
                ...
                $selection = new portfolio_format_leap2a_entry('newslettertempdiscussion' . $this->discussionid,
                    get_string('discussion', 'newslettertemp') . ': ' . $this->discussion->name, 'selection');
     ...
     * that corresponds to a single newslettertemp post,
    ...
    private function prepare_post_leap2a(portfolio_format_leap2a_writer $leapwriter, $post, $posthtml) {
        $entry = new portfolio_format_leap2a_entry('newslettertemppost' . $post->id,  $post->subject, 'resource', $posthtml);
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id])) {
            $leapwriter->link_files($entry, $this->keyedfiles[$post->id], 'newslettertemppost' . $post->id . 'attachment');
        }
    ...
    /**
     * this is a very cut down version of what is in newslettertemp_make_mail_post
     *
     * @global object
     * @param int $post
     * @return string
    ...
    private function prepare_post($post, $fileoutputextras=null) {
        ...
        $formattedtext = portfolio_rewrite_pluginfile_urls($formattedtext, $this->modcontext->id, 'mod_newslettertemp', 'post', $post->id, $format);
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id]) && count($this->keyedfiles[$post->id]) > 0) {
            ...
            $output .= '<br /><b>' .  get_string('attachments', 'newslettertemp') . '</b>:<br /><br />';
    ...
    function check_permissions() {
        ...
        if ($this->post) {
            return (has_capability('mod/newslettertemp:exportpost', $context)
                || ($this->post->userid == $this->user->id
                    && has_capability('mod/newslettertemp:exportownpost', $context)));
        }
        return has_capability('mod/newslettertemp:exportdiscussion', $context);
    }
    /**
     * @return string
     */
    public static function display_name() {
        return get_string('modulename', 'newslettertemp');
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class newslettertemp_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    public function __construct($browser, $course, $cm, $context, $areas, $filearea) {
        ...
        $this->course = $course;
        $this->cm = $cm;
        $this->component = 'mod_newslettertemp';
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if (($child = $this->browser->get_file_info($this->context, 'mod_newslettertemp', $this->filearea, $record->itemid))
