...
$url = new moodle_url('/mod/newslettertemp/user.php', array('id' => $userid));
if ($isspecificcourse) {
...
add_to_log(($isspecificcourse)?$courseid:SITEID, "newslettertemp", "user report", 'user.php?'.$url->get_query_string(), $userid);
...
if ($isspecificcourse) {
    ...
    $courses = newslettertemp_get_courses_user_posted_in($user, $discussionsonly);
}

// Get the posts by the requested user that the current user can access.
$result = newslettertemp_get_posts_by_user($user, $courses, $isspecificcourse, $discussionsonly, ($page * $perpage), $perpage);
...
if (empty($result->posts)) {
    ...
    $pagetitle = get_string('noposts', 'mod_newslettertemp');

    // Get the page heading
    if ($isspecificcourse) {
        $pageheading = format_string($course->shortname, true, array('context' => $coursecontext));
    } else {
        $pageheading = get_string('pluginname', 'mod_newslettertemp');
    }
    ...
    if ($iscurrentuser) {
        ...
        // current user.
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedbyyou', 'newslettertemp');
        } else {
            $notification = get_string('nopostsmadebyyou', 'newslettertemp');
        }
    } else if ($canviewuser) {
        ...
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedby', 'newslettertemp', $fullname);
        } else {
            $notification = get_string('nopostsmadebyuser', 'newslettertemp', $fullname);
        }
    } else {
        // Don't extend the navigation it would be giving out information that
        // the current uesr doesn't have access to.
        $notification = get_string('cannotviewusersposts', 'newslettertemp');
        if ($isspecificcourse) {
...
$discussions = $DB->get_records_list('newslettertemp_discussions', 'id', array_unique($discussions));

//todo Rather than retrieving the ratings for each post individually it would be nice to do them in groups
//however this requires creating arrays of posts with each array containing all of the posts from a particular newslettertemp,
...
foreach ($result->posts as $post) {
    if (!isset($result->newslettertemps[$post->newslettertemp]) || !isset($discussions[$post->discussion])) {
        // Something very VERY dodgy has happened if we end up here
        continue;
    }
    $newslettertemp = $result->newslettertemps[$post->newslettertemp];
    $cm = $newslettertemp->cm;
    $discussion = $discussions[$post->discussion];
    $course = $result->courses[$discussion->course];

    $newslettertempurl = new moodle_url('/mod/newslettertemp/view.php', array('id' => $cm->id));
    $discussionurl = new moodle_url('/mod/newslettertemp/discuss.php', array('d' => $post->discussion));
    ...
    if ($newslettertemp->assessed != RATING_AGGREGATE_NONE) {
        ...
        if ($newslettertemp->type == 'single' or !$post->discussion) {
            $ratingoptions->returnurl = $newslettertempurl;
        } else {
    ...
    $newslettertempname = format_string($newslettertemp->name, true, array('context' => $cm->context));
    ...
    if (!$isspecificcourse && !$hasparentaccess) {
        ...
        $fullsubjects[] = html_writer::link($newslettertempurl, $newslettertempname);
    } else {
        $fullsubjects[] = html_writer::tag('span', $courseshortname);
        $fullsubjects[] = html_writer::tag('span', $newslettertempname);
    }
    if ($newslettertemp->type != 'single') {
        ...
        if ($post->parent != 0) {
            ...
            if (!$isspecificcourse && !$hasparentaccess) {
                $fullsubjects[] .= html_writer::link(new moodle_url('/mod/newslettertemp/discuss.php', array('d' => $post->discussion, 'parent' => $post->id)), $postname);
            } else {
    ...
    $fulllink = html_writer::link($discussionurl, get_string("postincontext", "newslettertemp"));

    $postoutput[] = newslettertemp_print_post($post, $discussion, $newslettertemp, $cm, $course, false, false, false, $fulllink, '', null, true, null, true);
}

$userfullname = fullname($user);

if ($discussionsonly) {
    $inpageheading = get_string('discussionsstartedby', 'mod_newslettertemp', $userfullname);
} else {
    $inpageheading = get_string('postsmadebyuser', 'mod_newslettertemp', $userfullname);
}
if ($isspecificcourse) {
    $a = new stdClass;
    ...
    if ($discussionsonly) {
        $pagetitle = get_string('discussionsstartedbyuserincourse', 'mod_newslettertemp', $a);
    } else {
        $pagetitle = get_string('postsmadebyuserincourse', 'mod_newslettertemp', $a);
    }
} else {
...
if (!empty($postoutput)) {
    ...
    echo $OUTPUT->heading(get_string('nodiscussionsstartedby', 'newslettertemp', $userfullname));
} else {
    echo $OUTPUT->heading(get_string('noposts', 'newslettertemp'));
}
