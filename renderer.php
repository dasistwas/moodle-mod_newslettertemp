...
/**
 * This file contains a custom renderer class used by the newslettertemp module.
 *
 ...
 * is used by the newslettertemp module.
 *
 * @package mod-newslettertemp
 * @copyright 2009 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 **/
class mod_newslettertemp_renderer extends plugin_renderer_base {
    /**
     ...
     * @return string
     */
    public function subscriber_overview($users, $newslettertemp , $course) {
        $output = '';
        if (!$users || !is_array($users) || count($users)===0) {
            $output .= $this->output->heading(get_string("nosubscribers", "newslettertemp"));
        } else {
            $output .= $this->output->heading(get_string("subscribersto","newslettertemp", "'".format_string($newslettertemp->name)."'"));
            $table = new html_table();
            $table->cellpadding = 5;
            $table->cellspacing = 5;
    ...
    public function subscribed_users(user_selector_base $existingusers) {
        ...
        $output .= html_writer::tag('p', get_string('forcessubscribe', 'newslettertemp'));
